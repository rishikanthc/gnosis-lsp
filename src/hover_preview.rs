...
/// If one is found, it uses the provided database (via `db`) to search for a file whose virtual path matches.
    ...
    if let Some((_, _, virtual_path, _alias)) = parse_wiki_link(line, col) {
            ...
            .find(|f| f.virtual_path == virtual_path)
        {
            ...
            match fs::read_to_string(&file.path).await {
                Ok(content) => {
                    // Limit preview length if desired.
...
/// Parses a wiki‑link from a given line at the provided column position.
/// If a wiki‑link is found in the format `[[virtual_path|alias]]`, returns a tuple:
/// (start_index, end_index, virtual_path, alias)
pub fn parse_wiki_link(line: &str, col: usize) -> Option<(usize, usize, String, Option<String>)> {
    ...
    // Look forwards from `col` for the closing "]]".
    let end_rel = line[col..].find("]]")?;
    let end = col + end_rel;
    // Ensure that the hover position is within the wiki-link boundaries.
